...
def plot_percentile_with_gumbel():
    """Plot percentile graph of SWE maxima with Gumbel fit."""
    ...
    output_filename = os.path.join(output_folder, 'swe_maxima.png')
    plt.savefig(output_filename, dpi=300)
    # plt.show()


def plot_swe_timeseries():
    """Plot SWE time series with yearly maxima."""
    data = pd.read_csv(csv_file)
    data['time'] = pd.to_datetime(data['time'])
    data['water_year'] = data['time'].apply(lambda x: x.year + 1 if x.month >= 7 else x.year)
    data = data[(data['time'] >= '1958-07-01') & (data['time'] <= '2023-06-30')]
    ...
    output_filename = os.path.join(output_folder, f'swe_timeseries.png')
    plt.savefig(output_filename, dpi=300)
    # plt.show()
...
def plot_combined_timeseries_and_percentile():
    """Plot SWE time series and percentile plot with shared max SWE markers."""
    data = pd.read_csv(csv_file)
    data['time'] = pd.to_datetime(data['time'])
    data['water_year'] = data['time'].apply(lambda x: x.year + 1 if x.month >= 7 else x.year)
    data = data[(data['time'] >= '1958-07-01') & (data['time'] <= '2023-06-30')]
...
# Example: Run one or more plots by calling functions below
#plot_percentile_with_gumbel()
#plot_swe_timeseries()
