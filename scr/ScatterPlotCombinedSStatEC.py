...
def clean_station_name_and_assign_xk(name):
    ...
    # Assign xk value based on the name
    if 'Lom' in name:
        xk = 550
        xk2050 = 170
    elif 'Loppa' in name:
        ...
        xk = None  # Handle cases where the name doesn't match any criteria
    
    return name, xk, xk2050
...
output_folder = '/Users/kohlerm2/Documents/SnowAnalysis/Output/Computations/Analysis_withEC_comp'
...
for csv_file in csv_files:
    ...
    # Clean the station name and assign xk
    clean_name, xk, xk2050 = clean_station_name_and_assign_xk(station_name)
    ...
    # Get the Gumbel parameters using the gumbelei function
    a, u = gumbelei(1, 0.2, xk)
    
    # Create the figure for percentile plot
    plt.figure(figsize=(10, 6))
    
    # Scatter plot of SWE values
    scatter = plt.scatter(
        sorted_swe_values,
        percentiles,
        c=sorted_years,
    ...
    x_values = np.linspace(0, xk*1.2, 1000)  # x starts at 0
    y_gumbel = np.exp(-np.exp(-a * (x_values - u)))
    plt.plot(x_values, y_gumbel * 100, label='Gumbel Distribution', color='orange')
    ...
    # Add marker and lines for xk
    plt.axvline(x=xk, color='grey', linestyle='--', alpha=0.5)
    plt.axhline(y=np.exp(-np.exp(-a * (xk - u))) * 100, color='grey', linestyle='--', alpha=0.5)
    plt.scatter([xk], [np.exp(-np.exp(-a * (xk - u))) * 100], color='red', label=f'xk_EC = {xk} mm')
    # Plot Klima2050
    plt.scatter([xk2050], [np.exp(-np.exp(-a * (xk - u))) * 100], color='green', label=f'xk_2050 = {xk2050} mm')
    ...
    # Now create the comparison scatter plot for first and second 33 years
    ...
    if num_years >= 66:
        ...
        # Create the figure for comparison plot
        plt.figure(figsize=(10, 6))
        plt.scatter(
            swe_values_first,
            percentiles_first,
